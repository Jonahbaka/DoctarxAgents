...
export const walletTools: ToolDefinition[] = [
  ...
  {
    name: 'wallet_topup',
    description: 'Add funds to your DoctaRx wallet from any saved payment method (card or bank account).',
    category: 'wallet',
    ...
    execute: async (input, ctx) => {
      ...
      try {
        ...
        const body = new URLSearchParams({
          ...
          customer: customerId,
          payment_method: externalPmId,
          description: 'DoctaRx Wallet Top-Up',
          confirm: 'true',
          'automatic_payment_methods[enabled]': 'true',
          'automatic_payment_methods[allow_redirects]': 'never',
        });
  ...
  // ─── 10. Wallet Transfer (P2P) ─────────────────────────────
  {
    ...
    description: 'Transfer funds between DoctaRx user wallets (P2P). Instant, zero fees.',
    category: 'wallet',
  ...
  // ─── 11. Wallet Withdraw ───────────────────────────────────
  {
    ...
    description: 'Withdraw funds from your DoctaRx wallet to a bank account or debit card.',
    category: 'wallet',
    ...
    execute: async (input, ctx) => {
      ...
      for (const split of splits) {
        if (split.channel && split.channelId && sendTool) {
          const checkoutUrl = `${CONFIG.doctarx.apiUrl}/split-pay/${params.userId}/${split.name}`;
          const message = [
