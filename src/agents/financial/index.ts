...
export const creditBureauTool: ToolDefinition = {
  ...
  async execute(input: unknown, ctx: ExecutionContext): Promise<ToolResult> {
    ...
    ctx.logger.info(`Credit query: patient=${parsed.patientId} bureau=${parsed.bureau}`);
...
export const disputeFormFillerTool: ToolDefinition = {
  ...
  async execute(input: unknown, ctx: ExecutionContext): Promise<ToolResult> {
    ...
    ctx.logger.info(`Dispute filing: ${parsed.disputeType} to ${parsed.bureau} for patient ${parsed.patientId}`);
