    ...
    execute: async (input, ctx) => {
      ...
      try {
        ...
        const linkBody = new URLSearchParams({
          ...
          return_url: params.returnUrl || `${CONFIG.doctarx.apiUrl}/connect/return`,
          refresh_url: params.refreshUrl || `${CONFIG.doctarx.apiUrl}/connect/refresh`,
        });
  ...
  // ─── 7. Payment Request API ────────────────────────────────
  {
    ...
    execute: async (input, ctx) => {
      ...
      try {
        ...
        const checkoutUrl = `${CONFIG.doctarx.apiUrl}/pay/${piData.id}`;
